...
`,
  ...
  },
  {
    ...
    type: "objectives",
    phase: "Objectives",
    items: ["Model one list item as a type, then set up the component around it","Hold reviews in state and render it — rows when present, a message when empty","Wire controlled inputs so form fields live in React state","On submit, preventDefault, append one item to the list, and clear the form"],
  },
  {
    id: "step1",
    type: "question",
...
Your task: write \`type Review\` with \`id\` plus author, rating, body, then define and export ReviewInbox as a function component returning <div /> — every step from here on edits this same file.`,
    hint: `type Review = { id: string; author: string; rating: string; body: string; }
...
`,
      ...
      build: `type Review = { id: string; author: string; rating: string; body: string; }
...
export function ReviewInbox() {
  ...
  },
  {
    ...
    type: "question",
    phase: "Step 2 of 4",
    paal: `Hold reviews in state and render it — rows when present, a message when empty
...
Your task: hold reviews in state typed as Review[], starting empty, then render the empty message when reviews.length === 0 and the mapped rows (key={item.id}) otherwise.`,
    hint: `const [reviews, setReviews] = useState<Review[]>([]);
return reviews.length === 0 ? <p>No reviews yet.</p> : <ul>{reviews.map((a) => <li key={a.id}>{a.author}</li>)}</ul>;`,
...
React only redraws a component when the value it reads changes through React's own state — a plain variable can change without React ever finding out — and a zero-length array is a normal, common state that a bare map() renders as nothing at all, with no explanation for the user. Given both the sample rows and the empty case above, where does this growing array need to live, and what two branches does the render need to cover?`,
    ...
    why_this_matters: `Unanswered reviews cost trust. A list+form inbox is the light alternative to reputation suites. A plain array in a variable will not make React redraw, and a list that renders as literally nothing when empty looks broken — useState gives the screen something to watch, and branching on length before mapping is what keeps a brand-new list from looking like a bug.`,
