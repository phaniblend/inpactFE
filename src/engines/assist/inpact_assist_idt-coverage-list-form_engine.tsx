...
`,
  ...
  },
  {
    ...
    type: "objectives",
    phase: "Objectives",
    items: ["Model one list item as a type, then set up the component around it","Hold requests in state and render it — rows when present, a message when empty","Wire controlled inputs so form fields live in React state","On submit, preventDefault, append one item to the list, and clear the form"],
  },
  {
    id: "step1",
    type: "question",
...
Your task: write \`type CoverageRequest\` with \`id\` plus shiftId, reason, neededBy, then define and export CoverageDesk as a function component returning <div /> — every step from here on edits this same file.`,
    hint: `type CoverageRequest = { id: string; shiftId: string; reason: string; neededBy: string; }
...
`,
      ...
      build: `type CoverageRequest = { id: string; shiftId: string; reason: string; neededBy: string; }
...
export function CoverageDesk() {
  ...
  },
  {
    ...
    type: "question",
    phase: "Step 2 of 4",
    paal: `Hold requests in state and render it — rows when present, a message when empty
...
Your task: hold requests in state typed as CoverageRequest[], starting empty, then render the empty message when requests.length === 0 and the mapped rows (key={item.id}) otherwise.`,
    hint: `const [requests, setRequests] = useState<CoverageRequest[]>([]);
return requests.length === 0 ? <p>No open coverage requests.</p> : <ul>{requests.map((a) => <li key={a.id}>{a.shiftId}</li>)}</ul>;`,
...
React only redraws a component when the value it reads changes through React's own state — a plain variable can change without React ever finding out — and a zero-length array is a normal, common state that a bare map() renders as nothing at all, with no explanation for the user. Given both the sample rows and the empty case above, where does this growing array need to live, and what two branches does the render need to cover?`,
    ...
    why_this_matters: `Last-minute coverage is another list+form — request goes up, teammates claim later. A plain array in a variable will not make React redraw, and a list that renders as literally nothing when empty looks broken — useState gives the screen something to watch, and branching on length before mapping is what keeps a brand-new list from looking like a bug.`,
...
Your task: add one state value per field (shiftId, reason, neededBy), then wire each input's value and onChange to it.`,
...
A form field's text can live in the DOM itself (uncontrolled) or in React state (controlled) — a controlled input reads its value from state and writes every keystroke back into that same state. Where does each field's typed text need to live so what you type is exactly what submit will save?`,
    ...
    why_this_matters: `Controlled inputs use value from state and onChange to write back, keeping the form and the submit payload in sync. Last-minute coverage is another list+form — request goes up, teammates claim later.`,
...
Your task: on submit: call preventDefault, build a new CoverageRequest from the field state, add it to requests without mutating the old array, then clear the fields.`,
    hint: `e.preventDefault(); setRequests((prev) => [...prev, { id: String(Date.now()), shiftId, reason, neededBy }]); then clear fields.`,
...
Submitting an HTML form reloads the page by default; canceling that default lets your own handler run instead, and adding to a list in state means building a new array rather than mutating the old one. Given that, what has to happen, in order, when Request cover is used?`,
    ...
    why_this_matters: `preventDefault stops navigation; copying the old list plus one new item, then clearing fields, matches the design mock behavior. Last-minute coverage is another list+form — request goes up, teammates claim later.`,
...
`,
      ...
      watchOut: "Do not turn a single import or interface into its own lesson.",
      dryRun: "Write the same step for a different resource with the same shape.",
      build: `e.preventDefault(); setRequests((prev) => [...prev, { id: String(Date.now()), shiftId, reason, neededBy }]); then clear fields.`,
    },
  },
];
