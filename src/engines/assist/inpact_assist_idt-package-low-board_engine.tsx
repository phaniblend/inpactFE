...
`,
  ...
  },
  {
    ...
    type: "objectives",
    phase: "Objectives",
    items: ["Model one list item as a type, then set up the component around it","Hold packages in state and render only the matching rows — filtered, with an empty state","Wire controlled inputs so form fields live in React state","On submit, preventDefault, append one item to the list, and clear the form"],
  },
  {
    id: "step1",
    type: "question",
...
Your task: write \`type ServicePackage\` with \`id\` plus client, service, status, then define and export LowPackageBoard as a function component returning <div /> — every step from here on edits this same file.`,
    hint: `type ServicePackage = { id: string; client: string; service: string; status: string; }
...
`,
      ...
      build: `type ServicePackage = { id: string; client: string; service: string; status: string; }
...
export function LowPackageBoard() {
  ...
  },
  {
    ...
    type: "question",
    phase: "Step 2 of 4",
    paal: `Hold packages in state and render only the matching rows — filtered, with an empty state
...
Your task: hold packages in state typed as ServicePackage[], render packages.filter((a) => a.status === "low") mapped to rows (key={item.id}), and show the empty message when that filtered result has zero items.`,
    hint: `const [packages, setPackages] = useState<ServicePackage[]>([]);
const visible = packages.filter((a) => a.status === "low");
return visible.length === 0 ? <p>No low-balance packages.</p> : <ul>{visible.map((a) => <li key={a.id}>{a.client}</li>)}</ul>;`,
...
Filtering for display means computing a smaller array from the full one with .filter() before mapping — the state array itself never loses any rows, and a zero-length filtered result is still an empty case worth its own message. How do you keep the complete packages list in state, render only the subset above, and still show a clear message when that subset is empty?`,
    ...
    why_this_matters: `Owners upsell when a card is almost empty. Filter for display; keep full packages in state. Filtering in render lets users scan what matters without deleting other rows from state, and the empty case still needs its own message — an empty filtered view should not look like a broken screen.`,
...
Your task: add one state value per field (client, service, status), then wire each input's value and onChange to it.`,
...
A form field's text can live in the DOM itself (uncontrolled) or in React state (controlled) — a controlled input reads its value from state and writes every keystroke back into that same state. Where does each field's typed text need to live so what you type is exactly what submit will save?`,
    ...
    why_this_matters: `Controlled inputs use value from state and onChange to write back, keeping the form and the submit payload in sync. Owners upsell when a card is almost empty. Filter for display; keep full packages in state.`,
...
Your task: on submit: call preventDefault, build a new ServicePackage from the field state, add it to packages without mutating the old array, then clear the fields.`,
    hint: `e.preventDefault(); setPackages((prev) => [...prev, { id: String(Date.now()), client, service, status }]); then clear fields.`,
...
Submitting an HTML form reloads the page by default; canceling that default lets your own handler run instead, and adding to a list in state means building a new array rather than mutating the old one. Given that, what has to happen, in order, when Sell is used?`,
    ...
    why_this_matters: `preventDefault stops navigation; copying the old list plus one new item, then clearing fields, matches the design mock behavior. Owners upsell when a card is almost empty. Filter for display; keep full packages in state.`,
...
`,
      ...
      watchOut: "Do not turn a single import or interface into its own lesson.",
      dryRun: "Write the same step for a different resource with the same shape.",
      build: `e.preventDefault(); setPackages((prev) => [...prev, { id: String(Date.now()), client, service, status }]); then clear fields.`,
    },
  },
];
