...
`,
  ...
  },
  {
    ...
    type: "objectives",
    phase: "Objectives",
    items: ["Model one list item as a type, then set up the component around it","Hold templates in state and render it — rows when present, a message when empty","Wire controlled inputs so form fields live in React state","On submit, preventDefault, append one item to the list, and clear the form"],
  },
  {
    id: "step1",
    type: "question",
...
Your task: write \`type ReminderTemplate\` with \`id\` plus name, body, channel, then define and export TemplateDesk as a function component returning <div /> — every step from here on edits this same file.`,
    hint: `type ReminderTemplate = { id: string; name: string; body: string; channel: string; }
...
`,
      ...
      build: `type ReminderTemplate = { id: string; name: string; body: string; channel: string; }
...
export function TemplateDesk() {
  ...
  },
  {
    ...
    type: "question",
    phase: "Step 2 of 4",
    paal: `Hold templates in state and render it — rows when present, a message when empty
...
Your task: hold templates in state typed as ReminderTemplate[], starting empty, then render the empty message when templates.length === 0 and the mapped rows (key={item.id}) otherwise.`,
    hint: `const [templates, setTemplates] = useState<ReminderTemplate[]>([]);
return templates.length === 0 ? <p>No templates yet.</p> : <ul>{templates.map((a) => <li key={a.id}>{a.name}</li>)}</ul>;`,
...
React only redraws a component when the value it reads changes through React's own state — a plain variable can change without React ever finding out — and a zero-length array is a normal, common state that a bare map() renders as nothing at all, with no explanation for the user. Given both the sample rows and the empty case above, where does this growing array need to live, and what two branches does the render need to cover?`,
    ...
    why_this_matters: `Reusable wording is another list+form — same React skill, different nouns. A plain array in a variable will not make React redraw, and a list that renders as literally nothing when empty looks broken — useState gives the screen something to watch, and branching on length before mapping is what keeps a brand-new list from looking like a bug.`,
...
`,
      ...
      watchOut: "Do not turn a single import or interface into its own lesson.",
      dryRun: "Write the same step for a different resource with the same shape.",
      build: `const [templates, setTemplates] = useState<ReminderTemplate[]>([]);
return templates.length === 0 ? <p>No templates yet.</p> : <ul>{templates.map((a) => <li key={a.id}>{a.name}</li>)}</ul>;`,
    },
  },
  {
